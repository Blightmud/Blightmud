...
use pulldown_cmark_blightmud::resources::NoopResourceHandler;
use pulldown_cmark_blightmud::terminal::{TerminalProgram, TerminalSize};
use pulldown_cmark_blightmud::{Environment, Settings, Theme};
...
fn render_to_string<S: AsRef<str>>(markdown: S, settings: &Settings) -> String {
    ...
    pulldown_cmark_blightmud::push_tty(settings, &env, &NoopResourceHandler, &mut sink, parser)
        .unwrap();
